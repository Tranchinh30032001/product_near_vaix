...
## When client deposit fungible token to the smartcontract

## Make sure that the smartcontract have registered to that fungible token

--After you deploy the actionhup smartcontract you have to call the new function to initial values of SC
and set accountId of fungible that client want to deposit

- The new function take one paramater is the accountId of fungible token.

**syntax: near call $ID new '{"account_id": "fungibletoken$ID"} --accountId $ID**

--And then if client want to deposit to smartcontract => call the deposit_token function.

- The deposit_token take 3 paramter:
  *first: `amount` The amount that client want to deposit,
  *seconde: `category` The category that client want to deposit
  \*third: `msg` The message that client attched to deposit

\*\*\* Category is the symbol of the fungible token

**syntax: near call $ID deposit_token '{"amount": "Amount", "category": "Category", "msg": "..."} --accountID your_account$ID --amount 3 --gas 200_000_000_000_000**

-If the client want to check the amount that they have deposited => call get_client_balance function.
This function take 2 paramter:
*first: `account_id` this the account_id of client have depoisted that you want to check.
*second: `category` the category that you want to check

\*\*syntax: near view $ID get_client_balance '{"account_id": "AccountID", "category": "Category"}
